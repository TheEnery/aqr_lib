...
import '../core/error_correction.dart';
...
class DecodedBitStreamParser {
    ...
    try {
      ...
      do {
        ...
        if (bits.available < 4) {
          ...
          mode = Mode.fromBits(bits.getInt(4)); // mode is encoded by 4 bits
        }
        switch (mode) {
          ...
          case Mode.structuredAppend:
            ...
            symbolSequence = bits.getInt(8);
            parityData = bits.getInt(8);
            break;
          case Mode.eci:
          ...
          case Mode.hanzi:
            ...
            final subset = bits.getInt(4);
            final countHanzi =
                bits.getInt(mode.getCharacterCountBitLength(version));
          ...
          default:
            ...
            final count = bits.getInt(mode.getCharacterCountBitLength(version));
            switch (mode) {
              case Mode.numeric:
  ...
  static void _decodeByteSegment(
    ...
    if (currentCharacterSetECI == null) {
      ...
      // give a hint.
      encoding = StringUtils.guessCharset(readBytes);
    } else {
  ...
  static void _decodeAlphanumericSegment(
    ...
    while (count > 1) {
      ...
      final nextTwoCharsBits = bits.getInt(11);
    ...
    if (count == 1) {
      ...
      result.write(_toAlphaNumericChar(bits.getInt(6)));
    }
    ...
    if (fc1InEffect) {
      ...
      final alreadyDone = string.substring(0, start);
      final toProcess = string.substring(start);
      final processed = toProcess.replaceAllMapped(
        RegExp(r'%%|%'),
        (match) => match[0] == '%%' ? '%' : String.fromCharCode(0x1D),
      );

      result.clear();
      result.write(alreadyDone);
  ...
  static void _decodeNumericSegment(
    ...
    while (count >= 3) {
      ...
      final threeDigitsBits = bits.getInt(10);
    ...
    if (count == 2) {
      ...
      final twoDigitsBits = bits.getInt(7);
  ...
  static int _parseECIValue(BitReader bits) {
    final firstByte = bits.getInt(8);
    if ((firstByte & 0x80) == 0) {
      // just one byte
    ...
    if ((firstByte & 0xC0) == 0x80) {
      // two bytes
      final secondByte = bits.getInt(8);
      return (((firstByte & 0x3F) << 8) & 0xFFFFFFFF) | secondByte;
    }
    if ((firstByte & 0xE0) == 0xC0) {
      // three bytes
      final secondThirdBytes = bits.getInt(16);
