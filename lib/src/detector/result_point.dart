...
extension PointExtension on Point {
  Point operator +(Point other) => (x: this.x + other.x, y: this.y + other.y);
}

typedef FinderPoint = ({Point point, int moduleSize});

extension FinderPointExtension on FinderPoint {
  void doSmth() {
    print(point);
  }
}
...
void ok() {
  ...
  final fp = (point: p3, moduleSize: 4);
  fp.doSmth();

  var (v1, v2, v3) = (p, p3, p2);
  (v1, v2, v3) = (v3, v1, v2);
}
...
class ResultPoint {
  ...
  static double distance(ResultPoint pattern1, ResultPoint pattern2) {
    return distance2(pattern1.x, pattern1.y, pattern2.x, pattern2.y);
  }

  static double distance2(double aX, double aY, double bX, double bY) {
    final xDiff = aX - bX;
    final yDiff = aY - bY;
